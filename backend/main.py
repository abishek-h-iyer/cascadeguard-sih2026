...
def baseline_sensors(susceptibility_class):
    if susceptibility_class == "HIGH":
        base = {
            "rainfall_mm_h": 4,
            "soil_moisture": 0.42,
            "tilt_change_deg": 0.15,
            "upstream_rise_m_10m": 0.08,
            "downstream_rise_m_10m": 0.05,
        }
        ...
        base = {
            "rainfall_mm_h": 2,
            "soil_moisture": 0.30,
            "tilt_change_deg": 0.08,
            "upstream_rise_m_10m": 0.04,
            "downstream_rise_m_10m": 0.03,
        }

    return {
        "rainfall_mm_h": max(
            0.0,
            round2(base["rainfall_mm_h"] + jitter(1.5)),
        ),
        "soil_moisture": clamp(
            round2(base["soil_moisture"] + jitter(0.04)),
            0,
            1,
        ),
        "tilt_change_deg": max(
            0.0,
            round2(base["tilt_change_deg"] + jitter(0.04)),
        ),
        "upstream_rise_m_10m": max(
            0.0,
            round2(base["upstream_rise_m_10m"] + jitter(0.02)),
        ),
        "downstream_rise_m_10m": max(
            0.0,
            round2(base["downstream_rise_m_10m"] + jitter(0.02)),
        ),
    }
...
def jitter_existing(sensors):
    return {
        "rainfall_mm_h": max(
            0.0,
            round2(sensors["rainfall_mm_h"] + jitter(0.6)),
        ),
        "soil_moisture": clamp(
            round2(sensors["soil_moisture"] + jitter(0.01)),
            0,
            1,
        ),
        "tilt_change_deg": max(
            0.0,
            round2(sensors["tilt_change_deg"] + jitter(0.02)),
        ),
        "upstream_rise_m_10m": max(
            0.0,
            round2(sensors["upstream_rise_m_10m"] + jitter(0.01)),
        ),
        "downstream_rise_m_10m": max(
            0.0,
            round2(sensors["downstream_rise_m_10m"] + jitter(0.01)),
        ),
    }
...
class DashboardState:
    ...
    def _seed_events(self, timestamp_ms):
        self.events = [
            {
                "id": f"seed-{i}",
                "timestamp": timestamp_ms + offset * 1000,
                "message": message,
            }
        ...
        for zone_id in self.zone_ids:
            ...
            if prev:
                if prev["operational_status"] != final_status:
                    self._push_event(
                        f"{zone_id} status changed to {final_status}",
                        timestamp_ms,
                    )

                if (
                    not prev["cascade"]["possible_landslide"]
                    and result["cascade"]["possible_landslide"]
                ):
                    self._push_event(
                        f"Possible landslide detected at {zone_id}",
                        timestamp_ms,
                    )

                if (
                    not prev["cascade"]["possible_blockage"]
                    and result["cascade"]["possible_blockage"]
                ):
                    self._push_event(
                        f"Possible river blockage detected at {zone_id}",
                        timestamp_ms,
                    )

                if (
                    not prev["cascade"]["possible_surge"]
                    and result["cascade"]["possible_surge"]
                ):
                    self._push_event(
                        f"Downstream surge detected — propagating from {zone_id}",
                        timestamp_ms,
                    )

                if (
                    not prev["incoming_warning"]
                    and incoming_warning
                ):
                    self._push_event(
                        (
                            f"{zone_id} placed on "
                            f"{incoming_warning['status']} "
                            f"({incoming_warning['reason']})"
                        ),
                        timestamp_ms,
                    )
    ...
    def tick(self):
        ...
        for zone_id in self.zone_ids:
            if not self.overridden[zone_id]:
                self.raw[zone_id] = jitter_existing(
                    self.raw[zone_id]
                )

                self.sequences[zone_id] = (
                    self.sequences[zone_id] + 1
                ) % 256
...
app = FastAPI(
    ...
    description=(
        "Flash flood & cascade early-warning API "
        "for the Melamchi corridor."
    ),
    version="1.0.0",
    lifespan=lifespan,
)
...
class SensorOverride(BaseModel):
    rainfall_mm_h: Optional[float] = Field(
        default=None,
        ge=0,
        le=500,
    )
    ...
    tilt_change_deg: Optional[float] = Field(
        default=None,
        ge=0,
        le=90,
    )

    upstream_rise_m_10m: Optional[float] = Field(
        default=None,
        ge=0,
        le=50,
    )

    downstream_rise_m_10m: Optional[float] = Field(
        default=None,
        ge=0,
        le=50,
    )
    ...
    def require_at_least_one_field(self):
        if all(
            ...
            raise ValueError(
                "Provide at least one sensor field to override."
            )
...
class IngestRequest(BaseModel):
    packet: str = Field(
        min_length=1,
        description="Raw SOF|...|EOF framed sensor packet.",
    )
    ...
    if zone_id not in state.zone_ids:
        raise HTTPException(
            ...
            detail=(
                ...
                f"Known zones: {', '.join(state.zone_ids)}."
            ),
        )
...
async def reset_demo():
    ...
    return {
        "status": "RESET",
        "message": (
            "Demo state reset to baseline readings."
        ),
    }


@app.post("/zones/{zone_id}/override")
async def override_zone(
    zone_id: str,
    payload: SensorOverride,
):
    require_known_zone(zone_id)
    ...
    return {
        ...
        "message": (
            f"Manual override applied to {zone_id}; "
            "simulation paused for this zone."
        ),
        "zone": zone,
    }
...
async def clear_zone_override(zone_id: str):
    ...
    return {
        ...
        "message": (
            f"Override cleared for {zone_id}; "
            "resuming simulated live feed."
        ),
        "zone": zone,
    }
...
async def ingest_packet(
    ...
    async with state.lock:
        result = state.gateway.process_packet(
            payload.packet,
        ...
        if status == "DUPLICATE":
            return {
                "status": "DUPLICATE",
                "message": (
                    "Duplicate packet ignored — sequence "
                    f"{result['sequence']} already processed "
                    f"for node {result['node_id']}."
                ),
                **result,
            }
        ...
        if status == "WAITING":
            ...
            field = state.gateway.SENSOR_MAP[
                result["sensor_type"]
            ]
            ...
            return {
                "status": "WAITING",
                "message": (
                    f"Reading accepted for {zone_id}; "
                    f"waiting on {len(result['missing'])} "
                    "more sensor field(s) before evaluation."
                ),
                **result,
            }
        ...
        field = state.gateway.SENSOR_MAP[
            result["updated_sensor"]
        ]

        value = state.gateway.zone_state[
            zone_id
        ][field]
        ...
        return {
            "status": "EVALUATED",
            "message": (
                f"{zone_id} evaluated — alert level "
                f"{result['result']['alert_level']}."
            ),
            **result,
        }